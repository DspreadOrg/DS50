...
addHelpEntry("com","copen","NUM,BR=BR_AUTOMATIC,abortOnError=true","$CURRENTCONNECTION","Opens a CoolHost or a socket to a CoolHost on the COM port 'NUM' and with the baudrate 'BR'  and with NONE flowcontrol. Returns the constructed connection. Caution : if successful, this function modifies $CURRENTCONNECTION.")
def copen_none(arg1,arg2,abortOnError=true)
    goterror = gopen(abortOnError) {CHHostConnection.new(arg1,arg2,FLOWMODE_NONE)}
    return !goterror
end

addHelpEntry("com","copen","NUM,BR=BR_AUTOMATIC,abortOnError=true","$CURRENTCONNECTION","Opens a CoolHost or a socket to a CoolHost on the COM port 'NUM' and with the baudrate 'BR'  and with XONXOFF flowcontrol. Returns the constructed connection. Caution : if successful, this function modifies $CURRENTCONNECTION.")
def copen_xonxoff(arg1,arg2,abortOnError=true)
    goterror = gopen(abortOnError) {CHHostConnection.new(arg1,arg2,FLOWMODE_XONXOFF)}
    return !goterror
end
...
########handshake#############
addHelpEntry("chip","handShakeR","","","to adjuse whether the board and module communicate normally")
def handShakeR()
    pos = 0
    while (pos < 5)
        a = RI(0x03)
...
addHelpEntry("chip","handShake","","","to adjuse whether the board and module communicate normally")
def handShake()
...
addHelpEntry("chip","forcePanic","","","Send force panic command")
def forcePanic()
    handShake()
    $CURRENTCONNECTION.sendPacket(0xfd, [0x02])
end

addHelpEntry("chip","forcePanicR","","","Send force panic command,only for debughost")
def forcePanicR()
    handShakeR()
    $CURRENTCONNECTION.sendPacket(0xfd, [0x02])
end

addHelpEntry("chip","shutdown","mode","","Send shutdown command")
def shutdown(mode)
    $CURRENTCONNECTION.sendPacket(0xfd, [0x03, mode & 0xff, (mode >> 8) & 0xff])
end

addHelpEntry("chip","clearSysnv","","","Clear system NV")
def clearSysnv()
    $CURRENTCONNECTION.sendPacketSync(0xfd, [0x07])
end
...
class Array
    ...
    def from8to32bits()
        return self.pack("C*").unpack("L*")
    end
    
    def from8to16bits()
        return self.pack("C*").unpack("S*")
    end   
     
    def from32to8bits()
        return self.pack("L*").unpack("C*")
    end
 
    def from16to8bits()
        return self.pack("S*").unpack("C*")
    end   
