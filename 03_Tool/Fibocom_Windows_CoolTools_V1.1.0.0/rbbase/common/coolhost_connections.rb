...
public
   ...
   def initialize(p,b=BR_AUTOMATIC,x=CoolHostBase::FLOWMODE)
    ...
    def handShake()
        if( @flowmode == CoolHost::FLOWMODE_XONXOFF)
           CHST_HandShake(@handle)
        end
    end
    ...
    def getAccessMode()
        return CHST_GetAccessMode(@handle)
    end

    def setAccessMode(mode)
        CHST_SetAccessMode(@handle, mode)
    end
    ...
    def sendPacketSync(flowid,bytearray)
        CHST_SendBypassPacketSync(@handle,flowid,bytearray)
    end
