...
static int create_thread(const char* name, void (*entry)(void*), 
                        ...
                        int iRet=0;
						iRet=api_sys_threadCreate(entry, name, stack_size, NULL, priority, thread_id);
...
void app_main(void* param)
{
	...
	loadResource();
	ret = api_sys_readSn(tsn,&tsnlen,tusn,&tusnlen);
	...
	if (ret != 0) {
		api_sys_getSpTime(rtcTimeStrBcd);
		...
		{
			API_LOG_DEBUG("api_wifi_SNTPCfg 8");
			api_wifi_SNTPCfg(1,8,"0.pool.ntp.org","time.google.com", NULL);
		}
	...
	u32 btn_thread_id = 0;
	api_sys_threadCreate(app_trans_process, "app_trans",
		24*1024, NULL,
		API_PRIORITY_NORMAL,
		&btn_thread_id);

	api_sys_threadCreate(app_hibernate, "app_hibernate", 1024 * 4, NULL, API_PRIORITY_NORMAL-4, &thread_hibernateID);
...
exit:
	MainStatus=1;
	api_sys_threadDelete(thread_mainID);
	return;
}
...
void* appimg_enter(void* param)
{
	...
	int ilen=0;
	int ret;
	api_sys_watchdogDisable();
	prvInvokeGlobalCtors();
	api_sys_msleep(1000);
	p = api_sys_init(APP_VERSION);
	api_sys_msleep(1000);
	api_log_switch(0);
	api_keypad_setMode(1);
	lv_drv_init();
	api_audio_ttsSetSpeed(5000);
	rpc_protocol_task();
	mutex_lvgl = api_sys_mutextCreate();
	MainStatus=0;
