...
#define QRPAY_MUTEX_LOCK		 if(m_qrpay_mutext_lock == 0){m_qrpay_mutext_lock= api_sys_semNew(1);}if(m_qrpay_mutext_lock!=0)api_sys_semWait(m_qrpay_mutext_lock);
#define	QRPAY_MUTEX_UNLOCK	  if(m_qrpay_mutext_lock!=0)api_sys_semSignal(m_qrpay_mutext_lock);
...
int trans_thread_mutexLock(void) {
	...
	{
		m_transThread_mutext_lock = api_sys_semNew(1);
	}
	if (m_transThread_mutext_lock != 0)
		api_sys_semWait(m_transThread_mutext_lock);
	return 0;
}


int trans_thread_mutexUnlock(void) {
	if (m_transThread_mutext_lock != 0)api_sys_semSignal(m_transThread_mutext_lock);
	return 0;
}
...
void app_trans_init(void) {
	if (g_qrMutex == 0) {
		g_qrMutex = api_sys_mutextCreate();
	}
	if (g_qr_queue == 0)
		g_qr_queue = api_sys_queueCreate(1, sizeof(qr_queue_t));
}
...
int UnLockFlag=0;
 void WaitAppUnlock(void)
 {
	...
	while(UnLockFlag==1)
	{
		if (api_sys_tick() - startTick > 15000) {
			break;
		}
		fibo_watchdog_feed();
	    api_sys_msleep(10);
    ...
    int i=0;
	 ...
	 while (1)
	 {
		 ...
		 if (gTransThreadFlag) 
		 {
			 ...
			 tradeType = -1;
			 if (g_qr_queue != 0)
		 	 api_sys_queueClear(g_qr_queue);
 
			 {
				 api_sys_getHeapInfo(&memsize, &avail_size, &avail_block_size);
...
void app_transthread_paycheck(void* params) {
	...
	while (1) {
		...
		while (gTransThreadFlag && ((tradeType == -1) || (tradeType == 0))) {
			...
			{
				qr_data.tranState = 2;
				api_sys_queuePut(g_qr_queue, (void*)&qr_data, 1000);
				qrpay_flag = 2;
			}
...
void app_hibernate(void* param)
{
	...
	{
	    TMS_IsBusyWait();
	    hibernate_endTick = api_sys_tick();
		if ((hibernate_endTick - hibernate_startTick) > 1000 * m_sleep_time)
		{
			...
			if ((sleepstate == 0) && (gTransThreadFlag == false) && (api_audio_getState() == 0) && (api_audio_ttsGetState() == 0) && (TMS_IsBusy() == 0))
			{
				API_LOG_DEBUG("app is start sleep......");
				if (m_wake_type == 0) {
					api_sleep_cmd(SYS_CMD_SET_AUTO_SLEEP_AVAIBLE, 1, (u32)&hibernate_Func);
				}
				else if (m_wake_type == 1) {
					api_sleep_cmd(SYS_CMD_SET_NET_AUTO_SLEEP_AVAIBLE, 1, (u32)&hibernate_Func);
				}
			}
