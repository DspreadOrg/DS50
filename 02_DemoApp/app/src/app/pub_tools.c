...
int Public_ConnectFlow(unsigned char* pstr, int len, unsigned char* recvData)
{
	...
	if (com_mode == 0)//����ģʽ
	{
		...
		buf[len + 1] = 0x03;
		buf[len + 2] = 0x00;
		ret = api_usb_write(buf, len + 3);
		//__TRACEHEX__(buf, len + 3);
	}
	else
	{
		ret = api_usb_write(pstr, len);
	}
	//__TRACEASC__("api_usb_write=%d", ret);
	...
	while (1)
	{
		...
		Sys_Delay_ms(20);
		ret = api_usb_read(buf, 1024);
		// __TRACEASC__("api_usb_read=%d",ret);
		if (ret > 0)
		{
			break;
		}
		if (api_sys_tick() - startTick > 30 * 1000)
		{
			return -2;
		}
	}
