...
int CalcMac(char *outBuf, char *inBuf, int len, int mode)
{
	...
	if (mode == 0)
	{
		iRet = OsSecCalcMac(3, 1, 10, inBuf, len, outBuf);
		//__TRACEASC__("OsSecCalcMac iRet=%d", iRet);
	}
...
{
	int sRet;
	sRet = OsSecUpdateCipherKey(KeyType, 1, 1, mkindex, wkindex, pCipher, 16, 1, kcv, 4);
	//__TRACEASC__("OsSecUpdateCipherKey sRet=%d", sRet);
...
static int CardTask(unsigned char type, int nTimeOut)
{
	...
	while (1) {
		if (OsSysTick() - startTick > nTimeOut*1000) {
			break;
		}
