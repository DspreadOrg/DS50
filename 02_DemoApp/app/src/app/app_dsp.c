...
#define DSP_MUTEX_LOCK		 if(m_dsp_mutext_lock == 0){m_dsp_mutext_lock= api_sys_semNew(1);}if(m_dsp_mutext_lock!=0)api_sys_semWait(m_dsp_mutext_lock);
#define	DSP_MUTEX_UNLOCK	  if(m_dsp_mutext_lock!=0)api_sys_semSignal(m_dsp_mutext_lock);
...
static int write_transaction_info(void) {
	...
	strcat(filename, ".bin");
	int fp = api_file_open(filename, API_WRONLY | API_CREAT);
	int ret = api_file_write(fp, (char*)&m_tradeInfo, sizeof(dsp_tradeInfo_t));
	api_file_close(fp);
...
static int read_transaction_info(void) {
	...
	strcat(filename, DEVICEID);
	strcat(filename, ".bin");
	int fp = api_file_open(filename, API_RDONLY);
	if (fp >= 0) {
		int ret = api_file_read(fp, (char*)&m_tradeInfo, sizeof(dsp_tradeInfo_t));
		api_file_close(fp);
		return ret == sizeof(dsp_tradeInfo_t) ? 0 : -1;
	}
	return -1;
...
int app_dsp_login(char* mchntCd, char* deviceId, char* url, char* shopIdOut, char* staffIdOut,
	...
	char* headers = (char*)api_sys_malloc(1024);
	...
	for (i = 0; i < HTTP_TRYCNT; i++) {
		ret = api_http_handle(fullUrl, "POST", headers, NULL, bodyParamsStr, strlen(bodyParamsStr), &resp_content, &resp_content_len);
		API_LOG_DEBUG("app_dsp_login api_http_handle ret=%d,responseBuf(%d):%s\r\n", ret, resp_content_len, resp_content);
		if (ret == 0) {
			break;
		}
		if (resp_content) {
			api_sys_free(resp_content);
			resp_content = NULL;
		}
	}
	if (ret == 0) {
...
exit:
	if (headers) {
		api_sys_free(headers);
	}
	if (bodyParamsStr) {
		api_sys_free(bodyParamsStr);
	}
	if (root) {
		cJSON_Delete(root);
	}
...
int app_dsp_signin(char* mchntCd, char* deviceId, char* url) {
	...
	for (i = 0; i < HTTP_TRYCNT; i++) {
		ret = api_http_handle(fullUrl, "POST", headers, NULL, bodyParamsStr, strlen(bodyParamsStr), &resp_content, &resp_content_len);
		API_LOG_DEBUG("app_dsp_signin api_http_handle ret=%d,responseBuf(%d):%s\r\n", ret, resp_content_len, resp_content);
		if (ret == 0)
		{
			break;
		}
...
)
{
	...
	for (i = 0; i < HTTP_TRYCNT; i++) {
		ret = api_http_handle(fullUrl, "POST", headers, NULL, bodyParamsStr, strlen(bodyParamsStr), &resp_content, &resp_content_len);
		API_LOG_DEBUG("app_dsp_qrcodePay111 api_http_handle ret=%d,responseBuf(%d):%s\r\n", ret, resp_content_len, resp_content);
		if (ret == 0)
		{
			break;
		}
...
get_code_start:
	...
	headers = (char*)api_sys_malloc(1024);
	memset(headers, 0, 1024);
	...
	for (i = 0; i < HTTP_TRYCNT; i++) {
		ret = api_http_handle(fullUrl, "POST", headers, NULL, bodyParamsStr, strlen(bodyParamsStr), &resp_content, &resp_content_len);
		API_LOG_DEBUG("app_dsp_getQrcode api_http_handle ret=%d,responseBuf(%d):%s\r\n", ret, resp_content_len, resp_content);
		if (ret == 0) {
			break;
		}
		if (resp_content) {
...
int app_dsp_qrcodePayRefund(
	...
	app_lvgl_mainShow(DSP_WAITTING_IMG, 100, 100, "Connecting To Server", LV_COLOR_DSP_BLUE);
	ret = api_http_handle(fullUrl, "POST", headers, NULL, bodyParamsStr, strlen(bodyParamsStr), &resp_content, &resp_content_len);
	API_LOG_DEBUG("app_dsp_qrcodePayRefund traceNo:%s api_http_handle ret=%d,responseBuf(%d):%s\r\n", traceNo, ret, resp_content_len, resp_content);
	if (ret == 0) {
		respRoot = cJSON_Parse(resp_content);
		if (respRoot == NULL) {
			API_LOG_DEBUG("app_dsp_qrcodePay buf parse error\r\n");
...
int app_dsp_transactionFlow(
	...
	for (i = 0; i < HTTP_TRYCNT; i++) {
		ret = api_http_handle(fullUrl, "POST", headers, NULL, bodyParamsStr, strlen(bodyParamsStr), &resp_content, &resp_content_len);
		API_LOG_DEBUG("app_dsp_transactionFlow api_http_handle ret=%d,responseBuf(%d):%s\r\n", ret, resp_content_len, resp_content);
		if (resp_content_len) {
...
int app_dsp_settlement(
	...
	app_lvgl_mainShow(DSP_WAITTING_IMG, 100, 100, "Connecting To Server", LV_COLOR_DSP_BLUE);
	ret = api_http_handle(fullUrl, "POST", headers, NULL, bodyParamsStr, strlen(bodyParamsStr), &resp_content, &resp_content_len);
	API_LOG_DEBUG("app_dsp_checkPay traceNo:%s api_http_handle ret=%d,responseBuf(%d):%s\r\n", traceNo, ret, resp_content_len, resp_content);
	if (ret == 0) {
		respRoot = cJSON_Parse(resp_content);
		if (respRoot == NULL) {
			API_LOG_DEBUG("app_dsp_qrcodePay buf parse error\r\n");
...
int app_dsp_getStaticQrcode(
	...
	for (i = 0; i < HTTP_TRYCNT; i++) {
		...
		API_LOG_DEBUG("app_dsp_getStaticQrcode api_http_handle ret=%d\r\n", ret);
		if (resp_content_len) {
			API_LOG_DEBUG("app_dsp_getStaticQrcode responseBuf(%d):%s\r\n", resp_content_len, resp_content);
		}
		if (ret == 0) {
...
int app_dsp_cardPay(
	...
	char* resp_content = NULL;
	int resp_content_len = 0;
	char* headers = (char*)api_sys_malloc(1024*2);
	...
	icData=(char*)api_sys_malloc(emvInfo->cFieldlen*2+1);
	...
	for (i = 0; i < HTTP_TRYCNT; i++) {
		ret = api_http_handle(fullUrl, "POST", headers, NULL, bodyParamsStr, strlen(bodyParamsStr), &resp_content, &resp_content_len);
		API_LOG_DEBUG("app_dsp_cardPay api_http_handle ret=%d,responseBuf(%d):%s\r\n", ret, resp_content_len, resp_content);
		if (ret == 0) {
			break;
		}
		if (resp_content) {
...
int app_dsp_cardPayRefund(
	...
	for (i = 0; i < HTTP_TRYCNT; i++) {
		ret = api_http_handle(fullUrl, "POST", headers, NULL, bodyParamsStr, strlen(bodyParamsStr), &resp_content, &resp_content_len);
		API_LOG_DEBUG("app_dsp_cardPayRefund api_http_handle ret=%d,responseBuf(%d):%s\r\n", ret, resp_content_len, resp_content);
		if (ret == 0) {
			break;
		}
		if (resp_content) {
...
int app_dsp_terminalInit(char* url,
	...
	for (i = 0; i < HTTP_TRYCNT; i++) {
		ret = api_http_handle(fullUrl, "POST", headers, NULL, bodyParamsStr, strlen(bodyParamsStr), &resp_content, &resp_content_len);
		API_LOG_DEBUG("app_dsp_terminalInit api_http_handle ret=%d,responseBuf(%d):%s\r\n", ret, resp_content_len, resp_content);
		if (ret == 0) {
			break;
		}
		if (resp_content) {
...
int app_dsp_terminalStorage(char* url, char* msgOut) {
	...
	for (i = 0; i < HTTP_TRYCNT; i++) {
		ret = api_http_handle(fullUrl, "POST", headers, NULL, bodyParamsStr, strlen(bodyParamsStr), &resp_content, &resp_content_len);
		API_LOG_DEBUG("app_dsp_terminalStorage api_http_handle ret=%d,responseBuf(%d):%s\r\n", ret, resp_content_len, resp_content);
		if (ret == 0) {
			break;
		}
		if (resp_content) {
...
int app_dsp_qrcodePayClose(char * mchntCd,char *deviceId,char* url,char * outTradeNo,char* msgOut) {
	...
	for (i = 0; i < HTTP_TRYCNT; i++) {
		ret = api_http_handle(fullUrl, "POST", headers, NULL, bodyParamsStr, strlen(bodyParamsStr), &resp_content, &resp_content_len);
		API_LOG_DEBUG("app_dsp_qrcodePayClose api_http_handle ret=%d,responseBuf(%d):%s\r\n", ret, resp_content_len, resp_content);
		if (ret == 0) {
			break;
		}
		if (resp_content) {
