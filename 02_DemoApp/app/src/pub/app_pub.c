...
int app_write_record(TRANSINFO* new_record)
{
	...
	{
		next_write_index = 0;
		api_file_write(file_id, (unsigned char*)&next_write_index, INDEX_SIZE);
	}
	else if (file_size < INDEX_SIZE)
	{
		API_LOG_DEBUG("File size is less than index size. File might be corrupted.");
		api_file_remove(TRANSRECORD_FILE);
		api_file_close(file_id);
		return -1;
	}

	file_id = api_file_open(TRANSRECORD_FILE, API_RDWR | API_CREAT);
	if (file_id <= 0)
	{
		API_LOG_DEBUG("api_file_open fail ");
		return -1;
	}

	api_file_seek(file_id, 0, API_FS_SEEK_SET);
	api_file_read(file_id, (unsigned char*)&next_write_index, INDEX_SIZE);


	write_index = (next_write_index % MAX_RECORDS) * record_size + INDEX_SIZE;

	ret = api_file_seek(file_id, write_index, API_FS_SEEK_SET);
	if (ret < 0)
	{
		API_LOG_DEBUG("api_file_seek fail ");
		api_file_close(file_id);
		return -1;
	}

	ret = api_file_write(file_id, (char*)new_record, record_size);
	if (ret != record_size)
	{
		API_LOG_DEBUG("api_file_write fail ");
		api_file_close(file_id);
		return -1;
	}
	...
	ret = api_file_seek(file_id, 0, API_FS_SEEK_SET);
	if (ret < 0)
	{
	...
	ret = api_file_write(file_id, (char*)&next_write_index, INDEX_SIZE);
	if (ret != INDEX_SIZE)
	{
...
int app_read_all_records(TRANSINFO* records)
{
	...
	if (api_file_exist(TRANSRECORD_FILE) == 0)
		return -1;

	file_id = api_file_open(TRANSRECORD_FILE, API_RDONLY);
	if (file_id <= 0)
		return -1;
	...
	api_file_seek(file_id, INDEX_SIZE, API_FS_SEEK_SET);
	while (record_count < MAX_RECORDS && api_file_read(file_id, (unsigned char*)&records[record_count], record_size) == record_size)
	{
...
int app_wireless_get_csq(char* signalStatus)
{
	...
	ret = api_wireless_getCsq(&rssi, &ber);
	if (ret != 0)
	{
...
void app_seg_showMoney(char* str)
{
	...
	api_beep_play(1,100,0);
	if(strlen(str)<10)
	api_seg_showMoney(str);
}
...
int pub_getRtcTime(char *rtcTimeStr)
{
	...
	synclock=1;
	iRet=api_sys_getRtcTime(rtcTimeStr);
	synclock=0;
	return iRet;
}
...
int pub_GetNetWorkStatus()
{
	...
	do
	{
		if((api_wireless_getNetState()== 1) || api_esp_CheckApAsync(&stAPInfo) == 0)
		{
			ret = 0;
			break;
		}
...
{
	return api_ic_cardOpen(nSlotIndex);
}
int App_ICCardPresent(int nSlotIndex)
{
	...
	do
	{
		if(api_ic_cardGetStatus(nSlotIndex) != 0)
			break;
			api_sys_msleep(20);
		if(api_ic_cardPoweron(nSlotIndex,buf) != 0)
			break;
		ret = 1;
	} while (0);
...
{
	api_ic_cardPoweroff(nSlotIndex);
	api_ic_cardClose(nSlotIndex);
...
int App_PiccCardOpen()
{
	...
	do
	{
		if(api_rf_open() != API_OK)
			break;
		if(api_rf_poweron(PICC_TYPE_AB) != API_OK)
			break;
		ret = 1;
	} while (0);
...
void rf_test()
{
	...
	api_rf_open();
	api_rf_poweron(PICC_TYPE_A);
	cRet=api_rf_getStatus();
	if(cRet==3){
		API_LOG_DEBUG("--------------%s %d-----------",__FUNCTION__,__LINE__);
		if(!api_rf_activate())
		{
			API_LOG_DEBUG("--------------%s %d-----------",__FUNCTION__,__LINE__);
			if(!api_rf_exchangeApdu(&ApduSend,&ApduResp))
			{
				API_LOG_DEBUG("--------------%s %d-----------",__FUNCTION__,__LINE__);
