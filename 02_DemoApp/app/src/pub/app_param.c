...
{	
	...
	API_LOG_DEBUG("tms api_sec_updatePlainKey=[%d]",nRet);

	nRet= api_sec_updateCipherKey(0x10, 1, 1, 0, 10, PINKey, 16, 1, kcv, 4);

	API_LOG_DEBUG("tms api_sec_updateCipherKey=[%d]",nRet);

	if(api_file_exist(FILE_EMV_APP_PARAM)!=1 || api_file_exist(FILE_EMV_CAPK_PARAM)!=1)
	{
