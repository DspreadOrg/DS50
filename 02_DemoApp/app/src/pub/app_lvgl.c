...
{
	OsSysMutexLock(mutex_lvgl);
	lv_task_handler();
	OsSysMutexUnlock(mutex_lvgl);
}
...
void app_show_iconStatus(int type, int value)
{
		...
		if (value >= 0 && value <= 5)
		{
			...
			else
			{
				if (wire_img == 0 && (value != 0))
				{
					...
					OsLcdShowPng(6, 5, WIRE_IMG_WIDTH, WIRE_IMG_HEIGHT,
	...
	else if (type == 1 && (OsParamsGetNetMode() == 0))
	{
		if (value >= 1 && value <= 4)
		{
			if (wifi_img == 0)
			{
				OsLcdShowPng(40, 5, WIFI_IMG_WIDTH, WIFI_IMG_HEIGHT,
	...
	else if (type == 2)
	{
		if (value >= 0 && value <= 7)
		{
			if (soc_img == 0)
			{
				OsLcdShowPng(207, 8, SOC_IMG_WIDTH, SOC_IMG_HEIGT,
    ...
    if(freshflag==0)
	{
	   ...
	   freshentick=OsSysTick();
       freshflag=1;
	}
	if(OsSysTick()-freshentick > 1000)
	{
		freshflag=0;
	}
...
void app_lvgl_versionInfoShow()
{
	...
	OsSysGetFirmVersion(version, APP_VER);
	...
	memset(version, 0, sizeof(version));
	OsWifiGetVersion(version, sizeof(version));
	...
	memset(version, 0, sizeof(version));
	OsSysGetFirmVersion(version, SP_CORE_VER);
	...
	memset(version, 0, sizeof(version));
	OsSysGetFirmVersion(version, LIB_VER);
	...
	memset(version, 0, sizeof(version));
	OsSysGetFirmVersion(version, OS_VER);
	...
	lv_label_set_text_fmt(chargeVer, "%s", OsBattGetChargeState() == 1 ? "Charging" : (OsBattGetChargeState() == 2) ? "Full"
																																: "Not Charging");
	...
	if (OsBattGetChargeState() != 1)
	{
		...
		lv_label_set_text_fmt(batteryPercentVer, "%d%%", OsBattGetPercent());
...
void app_lvgl_paramQueryShow()
{
	...
	memset(buf, 0, sizeof(buf));
	sprintf(buf, "Volume Level: %d", OsAudioGetVolume(1));
