...
int app_button_semWait()
{
	...
	if (m_button_sem == 0)
	{
		m_button_sem = api_sys_semNew(0);
	}
	if (m_button_sem != 0)
		api_sys_semWait(m_button_sem);
...
int app_button_semSig()
{
	...
	if (m_button_sem != 0)
		api_sys_semSignal(m_button_sem);
	return m_button_sem == 0 ? -1 : 0;
}
...
static void VolumeOper(char type)
{
	...
	sprintf(buf, "%d", api_audio_getVolume(1));
	app_seg_showMoney(buf);

	if (type == 0)
	{
		API_LOG_DEBUG("audio_level_INC=[%d]", api_audio_getVolume(1));
		if (api_audio_getVolume(1) == MAX_AUDIO_VOLUME)
	...
	{
		API_LOG_DEBUG("audio_level_DEC=[%d]", api_audio_getVolume(1));
		if (api_audio_getVolume(1) == MIN_AUDIO_VOLUME)
...
void app_mon_button(void *param)
{
	...
	while (1)
	{
		...
		if (button != KEYPAD_NO_KEY) {
			hibernate_startTick = api_sys_tick();
	...
	api_sys_threadDelete(thread_buttonID);
}
...
static void handle_digit(u32 button)
{
		...
		(current_mode == MODE_SYS_PARAMS_SET_TIME))
	{
		if (strlen(inputBuf) < sizeof(inputBuf) - 2)
		{
			...
			{
				if (((api_sys_tick() - pressKeyTick) < 1500) && (button != PGDOWN) && (last_button == button))
				{
					if (strlen(inputBuf) > 0)
					{
						inputBuf[strlen(inputBuf) - 1] = 0;
...
static void handle_enter(void)
{
	...
	if (current_mode == MODE_NORMAL || current_mode == MODE_PLUS || current_mode==MODE_CARDPAY_SHOW)
	{
		...
		else if (gTransThreadFlag == false)
		{
			...
			if (atoi(amount) != 0 && button_length > 0)
			{
				...
				api_seg_showMoney(g_dynamic_amountFormat);
	...
	else if (current_mode == MODE_NETWORK_WIFI_INPUTPWD)
	{
		if (strlen(inputBuf) > 0 && strlen(currentSsid) != 0)
		{
			...
			ret = api_wifi_connect_hotspot(currentSsid, inputBuf);
			API_LOG_DEBUG("api_wifi_connect_hotspot ret:%d\r\n", ret);
			if (ret == 0)
			{
	...
	else if (current_mode == MODE_SYS_PARAMS_SET_TIME)
	{
		...
		if (strlen(inputBuf) == 14)
		{
			...
			ret = api_sys_setRtcTime(inputBuf);
			if (ret != 0)
			{
...
static void handle_func(void)
{
	...
	else
	{
		...
		else if (api_params_getNetMode() == 0)
		{
			if (api_wifi_get_connect_status() == 2)
			{
				app_dsp_play(APP_AUDIO_PLAY, "/ext/audio/english/dsp_netnormal.wav", "network connected", 0);
			}
			else
...
static void handle_sim_setting(void)
{
	...
	if (api_params_getNetMode() == 0)
	{
		api_params_setNetMode(1);
...
static void handle_wifi_device_setting(void)
{
	...
	while (1)
	{
		...
		lastTick = api_sys_tick();
		app_hibernate_reset(60);

		if ((lastTick - startTick > 15000 && current_mode == MODE_NETWORK_WIFI_CONFIG) ||
			current_mode == MODE_NETWORK_SET)
		{
			sleepstate = api_get_sleepstate();
			if (sleepstate == 1)
			{
				API_LOG_DEBUG("api_wifi_get_scan_results  wake......");
				api_AP_WakeUp_SP();
				api_sys_msleep(500);
			}
			current_mode = MODE_NETWORK_WIFI_CONFIG;
			...
			if (api_esp_CheckApAsync(&apInfo) == 0)
			{
				API_LOG_DEBUG("api_esp_CheckApAsync :[%s],[%d]\r\n", apInfo.cSsid, strlen(apInfo.cSsid));
				memset(currentConnectSsid, 0, sizeof(currentConnectSsid));

				memcpy(currentConnectSsid, apInfo.cSsid, strlen(apInfo.cSsid));
			}

			hotspot_totalCnt = api_wifi_get_scan_results(pstApList, sizeof(pstApList) / sizeof(pstApList[0]));
			trytimes++;

			API_LOG_DEBUG("api_wifi_get_scan_results hotspot_cnt:%d\r\n", hotspot_totalCnt);
		...
		if ((buttonwifi >= DIGITAL0 && buttonwifi <= DIGITAL9))
		{
			if (current_mode == MODE_NETWORK_WIFI_INPUTPWD)
			{
				if (strlen(inputBuf) < sizeof(inputBuf) - 2)
				{
					...
					{
						if (((api_sys_tick() - pressKeyTick) < 1500) && (buttonwifi != PGDOWN) && (last_buttonwifi == buttonwifi))
						{
							if (strlen(inputBuf) > 0)
							{
								inputBuf[strlen(inputBuf) - 1] = 0;
...
static void handle_wifi_phone_setting(void)
{
	...
	if (api_params_getNetMode() == 1)
	{
		...
		api_wifi_disconnect_hotspot();

	ret = api_wifi_WebNet_Connect(serialID, "", 1000 * 30);
	API_LOG_DEBUG("api_wifi_WebNet_Connect=[%d]", ret);
	...
	while (1)
	{
		...
		{
			API_LOG_DEBUG("api_wifi_WebNet_Connect CANCEL");
			ret = -1;
			break;
		}
		...
		if (api_sys_tick() - startTick > 1000 * 300)
		{
			API_LOG_DEBUG("api_wifi_WebNet_Connect timeout");
			ret = -1;
			break;
		}
		ret = api_wifi_get_connect_status();
		API_LOG_DEBUG("api_wifi_get_connect_status =[%d]", ret);
		if (ret == 2)
			break;
		api_sys_msleep(300);
	}
...
static void handle_wifi_firmware_update(void)
{
	...
	while (1)
	{
		if (api_sys_tick() - startTick > 1000 * 180)
		{
		...
		ret = api_wifi_USEROTA(WIFIUPDATEURL, strlen(WIFIUPDATEURL), 180000);
		API_LOG_DEBUG("api_wifi_USEROTA=[%d]", ret);
		if (ret == 0)
			break;
		api_sys_msleep(2000);
	}
...
static void handle_sync_time(void)
{
	...
	if (api_params_getNetMode() == 0)
	{
		...
		if (ret == 7)
		{
			...
			api_sys_setSpTime(&rtcTimeStrBcd[1]);
			app_lvgl_mainShow(DSP_SUCC_IMG, 100, 100,
