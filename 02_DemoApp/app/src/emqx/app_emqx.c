...
void get_sn(char *sn)
{
	...
	int sys_sn_len = 0;
	int cust_sn_len = 0;
	if(api_sys_readSn(sys_sn,&sys_sn_len,cust_sn,&cust_sn_len) == 0)
	{
...
static void emqx_state_fun(void*args){
	...
	case MQTT_SIG_CLOSE_RSP:
		...
		if (api_params_getNetMode() == 1) {//4g
			ret =  api_mqtt_connectEx(1,glb_sn, "q123b328.ala.cn-hangzhou.emqxsl.cn",8883, 0,			60, 4);
		}
	...
	case MQTT_SIG_CONNECT_RSP:
		if (state->ret == 1) {
			...
			ret += api_mqtt_subEx(1,topicbuf, 1, appMqttMessageArrived);
			API_LOG_DEBUG("api_mqtt_sub ret:%d\r\n");
			if (ret == 0) {
				state->ret = 1;
				state->type = 370;
...
int app_emqx_mqttConnect() {
	...
	get_sn(glb_sn);
	API_LOG_DEBUG("app_emqx_mqttConnect sn:%s\r\n", glb_sn);
	api_mqtt_setStateHandleEx(1, emqx_state_fun);
	api_mqtt_setUsrnamePwdEx(1, glb_sn, glb_sn);
	// if (api_params_getNetMode() == 1) {//4g
	// 	api_wireless_sslSetFile("/ext/key/server.pem", "/ext/key/client.pem", "/ext/key/client.key");
	// }
	// else {
	// 	/*	ret=api_wifi_LoadCrt(3, "/ext/key/server.pem", 3000);
	// 		API_LOG_DEBUG("api_wifi_LoadCrt ret:%d\r\n",ret);*/
	// 	ret=api_wifi_LoadCrt(4, "/ext/key/client.pem", 3000);
	// 	API_LOG_DEBUG("api_wifi_LoadCrt ret:%d\r\n", ret);
	// 	ret = api_wifi_LoadCrt(5, "/ext/key/client.key", 3000);
	// 	API_LOG_DEBUG("api_wifi_LoadCrt ret:%d\r\n", ret);
	// }
	ret = api_mqtt_connectEx(1,glb_sn, "q123b328.ala.cn-hangzhou.emqxsl.cn",8883, 0,			60, 4);
			//api_mqtt_connectEx(int id, char* clinetId, char* addr, unsigned short port, unsigned char cleansession,
